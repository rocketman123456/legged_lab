...
class SrobotWheelLeggedRewardCfg(RewardCfg):
    ...
    feet_force = RewTerm(
        func=mdp.body_force,
        weight=-3e-3,
        params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=".*ankle_roll.*"), "threshold": 500, "max_reward": 400},
    )
    feet_too_near = RewTerm(
        func=mdp.feet_too_near_humanoid, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*ankle_roll.*"]), "threshold": 0.2}
    )
    ...
    joint_deviation_legs = RewTerm(
        func=mdp.joint_deviation_l1, weight=-0.02, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_pitch.*", ".*_knee.*", ".*_ankle.*"])}
    )
...
class SrobotWheelLeggedFlatEnvCfg(BaseEnvCfg):
    ...
    robot = RobotCfg(terminate_contacts_body_names=[".*torso.*"], feet_body_names=[".*ankle_roll.*"])
...
class SrobotWheelLeggedRoughEnvCfg(SrobotWheelLeggedFlatEnvCfg):
    ...
    robot = RobotCfg(actor_obs_history_length=1, critic_obs_history_length=1, terminate_contacts_body_names=[".*torso.*"], feet_body_names=[".*ankle_roll.*"])
    reward = SrobotWheelLeggedRewardCfg(
